...
- **A pre-existing local-sandbox e2e flake, not a regression from this phase**: `e2e/phase5.spec.js`'s two "requires-acceptance checkbox" tests and the mandatory test 11 intermittently fail locally in this development container on `page.selectOption('#new-task-dep-assignee', ...)` timing out ("did not find some options"). Confirmed by reverting `src/dialogs/newTaskDialog.js` to its unmodified `main`-branch version and re-running the identical test — it fails the same way, ruling out anything introduced in this phase. Not reproduced when running `phase6.spec.js`'s own equivalent interaction (`ArrowDown` on the same select) in isolation. Given this container's repeated heavy Playwright/Chromium usage across a long session and the fact these exact tests passed in real CI when Phase 5 merged, this reads as environment-specific flakiness (a slow/contended headless Chromium in this sandbox) rather than a real bug — real CI (a fresh runner, fresh browser install per run) is the authoritative signal here, consistent with how every prior phase's e2e suite has been validated in this project.
