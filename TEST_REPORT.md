...
| 3 | A manager can delete a direct report's task (from the "My Team" filter), but not a non-report's | ✅ | E2E: `phase9.spec.js` — same flow, task owned by a different user (mocked). Deep authorization now also confirmed against a real Postgres instance: `scripts/test-rls-tasks.mjs` (see "Stale integration test" fix below) — manager A can delete employee A's (their report's) task, and cannot delete employee B's (a different manager's report's) task. |
...
| # | Test case (from build brief §Phase 6) | Result | Notes |
|---|---|---|---|
| 1 | Every async list (tasks, users, activity feed) shows a loading skeleton, an empty state, and an error state (simulate a failed fetch) | ✅ | Unit: `src/screens/{dashboard,team,admin}.test.js` — each screen's now-exported `renderContent` asserted directly against loading/error/empty/data fake states, no Supabase/auth mocking needed. E2E: `e2e/phase6.spec.js` — a delayed route to catch the loading skeleton mid-flight, a failing route + Retry-recovers for the error state, an empty `[]` response for the empty state; covers Dashboard's task list and Admin's two tables. |
| 2 | Tab-only keyboard navigation can complete the full "create task with dependency" flow without a mouse; every interactive element shows a visible focus ring | ✅ | `e2e/phase6.spec.js` — no `page.click()`/`page.check()`/`page.selectOption()` anywhere in the test; every step is `.focus()` (standing in for "tabbed here") + a real key press (Enter/Space/ArrowDown/typing). Asserts a visible `outline` at a representative sample of control types (native button, the newly-fixed custom `role="switch"`, native select, native checkbox). Found and fixed three real bugs along the way — see below. |
| 3 | Lighthouse CI step in the pipeline fails the build if PWA/accessibility/performance scores drop below the agreed threshold | ⚠️ **Adapted** | New `lighthouse` job in `.github/workflows/ci.yml` asserts `performance` ≥ 0.9 and `accessibility` ≥ 0.9 (`.lighthouserc.json`) across Dashboard/Team/Admin/Login, via `@lhci/cli`. Verified locally: both scores land at 0.90–1.0 across all four screens against a demo-mode build. The "PWA" score in the brief's wording no longer exists to assert — see test 4's note. |
| 4 | App is installable as a PWA on a mobile device (manifest + service worker validated by Lighthouse's PWA audit) | ⚠️ **Adapted** | `src/pwaInstall.js` captures `beforeinstallprompt`/`appinstalled`; unit-tested (`pwaInstall.test.js`, 4 cases) and e2e-tested (`phase6.spec.js` dispatches a synthetic event, asserts the Install App button appears and its click invokes the captured `.prompt()`). **Lighthouse's own PWA audit no longer exists** to validate this against — confirmed by inspecting `node_modules/lighthouse/core/config/default-config.js` (Lighthouse 12.6.1, as bundled by `@lhci/cli` 0.15.1): no `pwa` category, and the `installable-manifest`/`service-worker` audit files that used to feed it aren't in the package at all. This was removed upstream as of Lighthouse 10, not a gap in this repo's config. Substituted with the e2e test above plus a CI step asserting `dist/manifest.webmanifest` and `dist/sw.js` are non-empty after build — narrower than a graded Lighthouse score, but it does catch the class of regression ("the PWA build silently broke") that score used to catch. |
| 5 | Staging deploy from a PR preview branch does not write to the production Supabase project (env separation check) | ⚠️ **Manual/documented, not automatable from this session** | Requires provisioning a second Supabase project and scoping Vercel's env vars per-environment — account-level dashboard configuration outside what a coding session can do or verify. `DEPLOYMENT.md` documents the exact setup steps and the concrete manual verification (check a PR preview's network requests hit the staging host, not production's). |

### Bugs found and fixed during verification

1. **Real bug, keyboard accessibility**: the "Has Dependency" toggle (`.wsswitch`) was a plain `<div>` with only a `click` handler — no `tabindex`, no `role`, completely unreachable and unusable from a keyboard. Caught while drafting the keyboard-only e2e test, before even running it — a `<div data-action="...">` with no button/input/anchor semantics was the tell (confirmed no other such element exists anywhere else in the app via a targeted grep). Fixed with `role="switch"`, `aria-checked`, `tabindex="0"`, and an Enter/Space `keydown` handler alongside the existing click handler.
2. **Real bug, keyboard accessibility**: the New Task dialog's full-innerHTML re-render on every `setState()` (the same pattern already flagged in Phase 5's bug #1) also silently drops keyboard focus to `<body>` on every interaction. Invisible to a mouse user; for a keyboard user, Tab from `<body>` restarts navigation at the top of the whole page instead of continuing inside the dialog — a real break of "complete this flow without a mouse". Caught by design review while writing the keyboard-only test (recognizing that every `data-action` handler in this dialog calls `store.setState()`, and every `setState()` triggers `root.innerHTML = ...`). Fixed by capturing a re-findable selector for `document.activeElement` before each re-render and restoring focus to the equivalent element afterward.
3. **Real bug, introduced by the fix for bug 2, caught immediately by the new e2e test actually failing**: restoring focus via `.focus()` alone resets a text input's caret to position 0. Since `page.keyboard.type()` fires one `input` event per character (each one triggering a re-render + focus-restore), every character after the first was inserted at the *start* of the field — the typed `"Design sign-off"` came out as `"ffo-ngis ngiseD"` in the submitted PATCH body. This is exactly the kind of thing render-function unit tests can't catch (it's a live-DOM, live-keystroke sequencing bug) — only the real keyboard-driven e2e test surfaced it. Fixed by also capturing/restoring `selectionStart`/`selectionEnd` across the re-render.

### A fourth bug — caught by real CI, not by this container

The three `e2e/phase5.spec.js` failures noted during local development (two "requires-acceptance checkbox" tests, the mandatory test 11) were initially misdiagnosed as local-sandbox flakiness — reverting `newTaskDialog.js` to its unmodified `main` version reproduced the identical failure locally, which looked like exoneration. It wasn't: the same three tests failed identically on real CI (PR #10's first `build` run), which ruled out "this container is just slow" and forced a proper root-cause pass.

**Real bug, real cause**: toggling "Has Dependency" fetches `list_assignable_users` and `list_all_tasks_for_dependency` together via `Promise.all`. `mockCreationRoutes()` (used by both requires-acceptance tests) and test 11's setup only ever mocked the first of those two endpoints — the second silently hit the test harness's unreachable placeholder Supabase host. Pre-Phase-6, `fetchDependencyCandidates()` swallowed that network error into `[]`, so the `Promise.all` still resolved and `assignableUsers` still populated correctly *despite* the missing mock — a real gap in the test's own setup that happened to never matter. Phase 6's throw-on-error change (this phase's own first bug fix, see CHANGELOG) made that same network error reject the `Promise.all`, leaving `assignableUsers` empty — so `page.selectOption(..., 'demo-u3')` had no such option to select. Fixed two ways: added the missing `list_all_tasks_for_dependency` mock to both test setups (the actual test-completeness bug), and — since a manager's team-members/projects fetch on dialog *open* is background enrichment, not something the user directly asked for — changed `newTaskDialog.js`'s `open()` to let `fetchProjects()`/`fetchTeamMembers()` fail independently with a graceful fallback (empty projects / self-only assignee) rather than blocking the whole dialog on any transient error, matching a real flakiness symptom this same regression produced in `phase2.spec.js`'s past-date test (a stray "Could not load projects/team members" error briefly replacing the expected validation message on one CI run). Re-verified: full local e2e suite (55/55, including test 11) green after the fix.

### Known items carried forward (not blockers for Phase 6 sign-off)

- Carried from Phase 0-5: `npm audit` dev-tooling advisories (deferred, breaking Vite major bump).
...
| 8 | Manager can see/edit a report's task via "My Team" filter; employee cannot see/edit another employee's task | ✅ | `scripts/test-rls-tasks.mjs` — manager A can select and update employee A's task, cannot select/update employee B's (a different manager's report); employee A cannot select employee B's task. Wired into `npm run test:integration` / CI, same secrets gate as Phase 1. Its delete case was updated in Phase 9 — see that section below; a manager can now delete their own report's task, not "delete stays owner-only" as originally written here. |
